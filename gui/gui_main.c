...
#define MAX_MESSAGE_LEN 2048
  ...
  if (buffer_id < 0) buffer_id = 0;
  if (buffer_id > 1) buffer_id = 1;
	      ...
	      int result)
{
  ...
  switch (result)
    ...
    case RET_BAD_FORMAT:   id = IDS_BAD_FORMAT_ERR; break;
    case RET_BAD_DEVICE:   id = IDS_BAD_DEVICE_ERR; break;
    case RET_NO_SPACE:     id = IDS_NO_SPACE_ERR; break;
    case RET_BAD_APA:      id = IDS_BAD_APA_ERR; break;
    case RET_NO_MEM:       id = IDS_NO_MEM_ERR; break;
    case RET_PART_EXISTS:  id = IDS_PART_EXISTS_ERR; break;
    case RET_NOT_COMPAT:   id = IDS_NOT_COMPAT_ERR; break;
    case RET_NOT_ALLOWED:  id = IDS_NOT_ALLOWED_ERR; break;
    case RET_CROSS_128GB:  id = IDS_DATA_CROSS_128GB; break;
    case RET_ERR:          show_error (parent); return; /* OS error */
    default:               id = IDS_UNKNOWN_ERR; break;
    }
...
dlg_get_source (HWND dlg, char source [MAX_PATH])
{
  if (IsDlgButtonChecked (dlg, IDC_ISO_OPT) == BST_CHECKED)
    ...
    { /* if there are no optical drives radio button would be disabled */
...
dlg_refresh_hdd_info (HWND dlg)
{
  ...
  /* clean-up */
  hdl_glist_free (games_); games_ = NULL;
...
void
dispose_progress (progress_t *pgs)
{
  HWND dlg = GetParent (progress_dlg);
...
dlg_init (HWND dlg)
{
  ...
  if (fill_ps2hdd_combo (dlg) == RET_OK)
    ...
    { /* disable "local HDD" at all if not found */
  ...
  switch (msg)
	...
	{ /* button clicked */
	  switch (LOWORD (wparam)) /* control ID */
	    ...
	    case IDC_NET_U2LINK:
	      { /* attempt to connect to the networking server */
		int result = dlg_refresh_hdd_info (dlg);
		if (result != RET_OK)
		  show_error_2 (dlg, result);
	      }
