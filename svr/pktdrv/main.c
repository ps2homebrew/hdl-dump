...
static int FlushInputQueue(void){
	if(RxPbuf.length>0){
		if(icmp_probe(FrameBuffer, RxPbuf.length)) icmp_dispatch(FrameBuffer, RxPbuf.length);
		else if(arp_probe(FrameBuffer, RxPbuf.length)) arp_dispatch(FrameBuffer, RxPbuf.length);
		else if(udp_probe(FrameBuffer, RxPbuf.length)) udp_dispatch(FrameBuffer, RxPbuf.length);
