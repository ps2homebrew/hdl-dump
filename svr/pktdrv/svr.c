         ...
         unsigned long len)
{
    ...
    dataptr = (void *)chain[0].data;
    memcpy(&dataptr[chain[0].data_len], payload, len);

    /* sizes for both chain entries are aligned to 4 bytes */
    nic_send_wait(tmp, chain[0].data_len + chain[1].data_len);
         ...
         unsigned char count)
{
    ...
    else {                    /* read requested data */
        if (wr_dirty_count) { /* flush write buffer before read operation */
          ...
          const void *buf)
{
    ...
    if ((wr_sector % wr_buf_size) == 0 && /* buffer contains any data */
        ...
        if (wr_dirty_count > 0) { /* sync write buffer */
            int result = svr_sync_buffer();
            if (result != 0)
                return (svr_return(uf, req, result));
        }
...
int svr_request(const udp_frame_t *uf)
{
    ...
    switch (p->command) {
        ...
        case cmd_shutdown: { /* dev9 shutdown; borrowed from ps2link */
            dev9IntrDisable(-1);
            dev9Shutdown();
