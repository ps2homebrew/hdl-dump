		     ...
		     char source [MAX_PATH])
{
  ...
  int result;
  char *p = line;
  while (is_space_or_tab (*p)) ++p;
  line = p;
  while (!is_space_or_tab (*p) && *p != '\0') ++p;
  *p++ = '\0';
  ...
  *p++ = '\0';
  ...
  if (result == RET_OK)
	...
	result = (tolower (text_mode [0]) == 'm' &&
		  ...
		  tolower (text_mode [3]) == 'e') ? RET_OK : RET_BAD_COMPAT;
      if (result == RET_OK)
	{
	   ...
	   data_mode_t *mode)
{
  ...
  if (result == OSAL_OK)
      ...
      if (result == OSAL_OK)
	{
	  ...
	  if (result == OSAL_OK)
	    { /* validate that 3rd line is an index line */
	      line = strtok (NULL, "\r\n");
	      if (line != NULL && *line != '\0')
