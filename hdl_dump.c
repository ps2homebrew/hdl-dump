...
static void
show_dm_toc(const dict_t *config, const apa_toc_t *toc, const char *device_name, const char *part_name)
{
    ...
    for (i = 0; i < slice->part_count; ++i) {
        ...
        if (once && strncmp(part->id, part_name, PS2_PART_IDMAX))
            continue;
         ...
         const char *path_name)
{
    ...
    if (result == RET_OK && toc != NULL) {
        if (option == NULL)
            ...
            show_dm_toc(config, toc, device_name, path_name);
        apa_toc_free(toc);
    }
    return (result);
}
    ...
    static const struct help_entry_t
    {
    ...
    } help[] =
        {
            ...
            {CMD_TOC, "device [option]",
             ...
             "option:\n\n"
             "--dm [name]: print \"concise\" tables for linux device-mapper to be used like that:\n"
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1) {
        ...
        if (0)
            ...
            if (argc == 5)
                part_name = argv[4];
            handle_result_and_exit(show_toc(config, argv[2], option, part_name), argv[2], NULL);
        }
